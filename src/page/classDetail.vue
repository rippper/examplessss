...
<template>
  <div class="class_detail">
    <div class="sign_bg">
      ...
      <i v-if="isCdsxx" class="webapp webapp-scanning scan" @click.stop="scan(classDetail.Status)"></i>
    </div>
    ...
    <div class="sign_detail" v-if="isCdsxx">
...
<script>
  ...
  import { isUseWxSdk, appId } from '../service/config'
  ...
  export default {
    ...
    created () {
      ...
      let isCdsxx = this.$route.query.isCdsxx
      this.isCdsxx = typeof isCdsxx === 'string' ? isCdsxx === 'true' : isCdsxx
    },
    mounted () {
      console.log(this.userAgent, this.userAgent.weixin && isUseWxSdk)
      if (this.userAgent.weixin && isUseWxSdk) {
    ...
    methods: {
      ...
      async getClassDetail () {
        ...
        if (data.Type == 1) {
          ...
          this.isJoinClass = data.Data.Status !== 'UnJoin'
        }
      },
      //签到列表
      async getSignList () {
